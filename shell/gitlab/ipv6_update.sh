#!/usr/bin/bash    
dnspod_ddnsipv6_id="485292" #【API_id】将引号内容修改为获取的API的ID
dnspod_ddnsipv6_token="ce0cbc552c7f3442b2acf9ffe913a461" #【API_token】将引号内容修改为获取的API的token
dnspod_ddnsipv6_ttl="600" # 【ttl时间】解析记录在 DNS 服务器缓存的生存时间，默认600(s/秒)
dnspod_ddnsipv6_domain='tf1992.cn' #【已注册域名】引号里改成自己注册的域名
...
ipv6_list=`ip addr show $local_net | grep "inet6.*global" | awk '{print $2}' | awk -F"/" '{print $1}' | head -n2 | tail -n1` || die
